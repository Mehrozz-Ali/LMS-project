...
const ErrorHandler_1 = __importDefault(require("../utilis/ErrorHandler"));
const catchAsyncErrors_1 = require("../middleware/catchAsyncErrors");
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const ejs_1 = __importDefault(require("ejs"));
const path_1 = __importDefault(require("path"));
const sendMail_1 = __importDefault(require("../utilis/sendMail"));
const jwt_1 = require("../utilis/jwt");
const redis_1 = require("../utilis/redis");
...
exports.activationUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
...
exports.updateAccessToken = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        await redis_1.redis.set(user._id, JSON.stringify(user), "EX", 604800); //after 7 days data will be expired from redis
        next();
    }
    catch (error) {
        return next(new ErrorHandler_1.default(error.message, 400));
...
exports.socialAuth = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (!user) {
            const newUser = await user_model_1.default.create({ email, name, avatar: { public_id: "", url: avatar } });
...
exports.updateUserProfilePicture = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const user = await user_model_1.default.findById(userId);
        if (avatar && user) {
            // if user have avatar then call this if statement
            if (user?.avatar?.public_id) {
                // first delete the existing image from cloudinry
...
exports.updateUserRole = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        await (0, user_service_1.updateUserRoleService)(res, id, role);
    }
    catch (error) {
        return next(new ErrorHandler_1.default(error.message, 500));
    }
