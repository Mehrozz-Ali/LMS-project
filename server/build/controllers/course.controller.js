...
exports.getSingleCourse = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (isCacheExist) {
            ...
            await redis_1.redis.set(courseId, JSON.stringify(course), "EX", 604800); // data will expire after 7 days from redis 
            res.status(200).json({
                success: true,
                course,
            });
...
exports.addReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (course) {
            course.ratings = avg / course.reviews.length; // one e.g. we have 2 revies so 1st is 5 and 2nd is 4 so math working like this 9/2=4.5 ratng
        }
        await course?.save();
