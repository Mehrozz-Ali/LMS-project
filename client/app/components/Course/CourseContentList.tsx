        ...
        <div className={`mt-[15px] w-full ${!props.isDemo && 'ml-[-30px]  sticky top-24 left-0 z-30'}`}>
            {videoSection.map((section: string, sectionIndex: number) => {
                ...
                const sectionVideoLength: number = sectionVideos.reduce(
                    (totalLength: number, item: any) => totalLength + Number(item.videoLength || 0), 0
                );

                console.log(sectionVideos.map(v => [v.videoLength, typeof v.videoLength]));
                        ...
                        {isSectionVisible && (
                            <div className="w-full">
                                {sectionVideos.map((item: any, index: number) => {
                                    const videoIndex: number = sectionStartIndex + index; //calculating the video index within the overall list
                                    const contentLength: number = item.videoLength / 60;
                                    return (
                                        <div className={`w-full ${videoIndex === props.activeVideo ? "bg-slate-800" : ""
